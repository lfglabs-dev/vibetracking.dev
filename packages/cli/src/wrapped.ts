...
const ASSETS_BASE_URL = "https://vibetracking.dev/assets/logos";
...
const VIBETRACKING_LOGO_SVG_URL = "https://vibetracking.dev/vibetracking-logo.svg";
const VIBETRACKING_LOGO_PNG_SIZE = 400;

function getImageCacheDir(): string {
  return path.join(os.homedir(), ".cache", "vibetracking", "images");
}

function getFontCacheDir(): string {
  return path.join(os.homedir(), ".cache", "vibetracking", "fonts");
}
...
async function generateWrappedImage(data: WrappedData, options: { short?: boolean; includeAgents?: boolean; pinSisyphus?: boolean } = {}): Promise<Buffer> {
  ...
  const vibetrackingLogoHeight = 72 * SCALE;
  ...
  try {
    const logoPath = await fetchSvgAndConvertToPng(VIBETRACKING_LOGO_SVG_URL, "vibetracking-logo@2x.png", VIBETRACKING_LOGO_PNG_SIZE * SCALE);
    const vibetrackingLogo = await loadImage(logoPath);
    const logoWidth = (vibetrackingLogo.width / vibetrackingLogo.height) * vibetrackingLogoHeight;

    ctx.fillStyle = COLORS.textSecondary;
    ctx.font = `${18 * SCALE}px Figtree, sans-serif`;
    ctx.fillText("vibetracking.dev", PADDING, footerBottomY);

    const logoY = footerBottomY - 18 * SCALE - 16 * SCALE - vibetrackingLogoHeight;
    ctx.drawImage(vibetrackingLogo, PADDING, logoY, logoWidth, vibetrackingLogoHeight);
  } catch {
  }
...
export async function generateWrapped(options: WrappedOptions): Promise<string> {
  ...
  const outputPath = options.output || `vibetracking-${data.year}-wrapped.png`;
